...
```{r}
summary_table <- readRDS("./summary_table.RDS") %>% clean_names()
diopt_mousetofly <- qread("./data/DIOPT/mouse96-to-fly/DIOPT_mouse-to-fly.qs")

## Explore diopt conversion table
diopt_mousetofly %>% 
  filter(!is.na(diopt_score)) %>%
  ggplot(aes(x = as.factor(diopt_score), fill = diopt_score)) +
  geom_bar(width = 0.8) +
  geom_vline(xintercept = 7.5, colour = "gray50", linetype = "dashed") + 
  scale_fill_viridis_c(direction = -1) +
  labs(title = "Mouse-to-fly conversion - homolog score",
       x = "DIOPT score",
       y = "Frequency") + 
...
qsave(df_raw_merge, "./data/glia_localised_RNA_dmel.qs")
...
```{r}
glia_localised_rna_dmel <- qread("./data/glia_localised_RNA_dmel.qs")

glia_localised_rna_dmel %>%
