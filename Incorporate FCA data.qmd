...
```{r}
df_raw_merge <- qread("~/Google Drive/df_raw_merge.qs")

all_glia_dgem_by_gene %>% pull(gene_name) %>% unique() -> a
df_raw_merge %>% pull(dmel_gene_name) %>% unique() -> b

intersect(b, a) %>% length
    ...
    imap_dfr(~
               tibble(
                 ...
                 gene_count = c(
                   ...
                   filter(df_localised_FCA, !!sym("subperineurial glial cell") == TRUE) 
